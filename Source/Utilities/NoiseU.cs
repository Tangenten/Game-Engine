...
namespace RavUtilities {
	public class NoiseU {
		private static ThreadLocal<OpenSimplex2F> n = new ThreadLocal<OpenSimplex2F>(() => new OpenSimplex2F(0));

		public static double Get(double x, double y, double height = 1.0, double scale = 1.0, double amplitude = 1.0, double frequency = 1.0, int octaves = 1, double persistence = 1.0, double exponentiation = 1.0, double lacunarity = 1.0) {
			double xs = x / scale;
			double ys = y / scale;
			double G = Math.Pow(2.0, -persistence);

			double normalization = 0;
			double total = 0;
			for (int o = 0; o < octaves; o++) {
				double noiseValue = n.Value.Noise2(xs * frequency, ys * frequency);
		...
		public static double Get(double x, double y, double z, double height = 1.0, double scale = 1.0, double amplitude = 1.0, double frequency = 1.0, int octaves = 1, double persistence = 1.0, double exponentiation = 1.0, double lacunarity = 1.0) {
			double xs = x / scale;
			double ys = y / scale;
			double zs = z / scale;
			double G = Math.Pow(2.0, -persistence);
			...
			double total = 0;
			for (int o = 0; o < octaves; o++) {
				double noiseValue = n.Value.Noise3_Classic(xs * frequency, ys * frequency, zs * frequency);
				total += noiseValue * amplitude;
				normalization += amplitude;
				amplitude *= G;
				frequency *= lacunarity;
	...
	public class OpenSimplex2F {
		...
		public OpenSimplex2F(long seed) {
			...
			this.permGrad4 = new Grad4[PSIZE];
		...
		private struct LatticePoint4D {
			...
			public LatticePoint4D(int xsv, int ysv, int zsv, int wsv) {
				...
				this.dz = -zsv - ssv;
				this.dw = -wsv - ssv;
				this.xsi = this.xsi = 0.2 - xsv;
				this.ysi = this.ysi = 0.2 - ysv;
				this.zsi = this.zsi = 0.2 - zsv;
				this.wsi = this.wsi = 0.2 - wsv;
